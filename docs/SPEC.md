...
`reference/` フォルダ（`.gitignore` 対象）にテスト用サンプルを格納。

| ファイル | 内容 |
|---|---|
| `_89R3389.CR2` | Canon EOS 1DX で撮影したRAWファイル |
| `_89R3389.png` | 上記をLightroomで**無修正**で書き出したPNG（現像品質の比較基準） |

現像結果の色再現・解像感の評価はこのPNGを基準として行う。

## 出力データ
`output/` デバッグや確認用の画像を出力するフォルダ
